...
export function DressCard({ dress }: DressCardProps) {
  return (
    <Link href={`/dresses/${dress.id}`}>
      <div className="group cursor-pointer rounded-xl bg-card border border-border overflow-hidden hover:shadow-xl hover:shadow-primary/5 transition-all duration-500 flex flex-col h-full hover:-translate-y-1">
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          <div className="mt-auto pt-4 border-t border-border flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground uppercase tracking-wider">{dress.category === 'evening' ? 'سهرة' : 'ناعم'}</span>
          </div>
        </div>
      </div>
    </Link>
