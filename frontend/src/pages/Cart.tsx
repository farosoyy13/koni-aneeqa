    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 bg-[#050505]">
        ...
        <p className="text-white/60 mb-8 text-center max-w-md">
          لم تقومي بإضافة أي منتجات بعد. تألقي بأناقة واكتشفي مجموعتنا الفاخرة.
        </p>
        <Link href="/dresses">
          <Button
            size="lg"
            className="rounded-full px-8 font-bold bg-[#d4af37] text-black hover:bg-[#b8860b]"
          >
            تصفح التشكيلة
          </Button>
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-[#0b0b0b] rounded-2xl border border-[#d4af37]/20 p-4 sm:p-6 flex flex-col sm:flex-row gap-6 items-center sm:items-start shadow-lg"
              >
                <div className="w-32 h-40 shrink-0 rounded-xl overflow-hidden bg-[#1a1a1a]">
                  <img
                    src={item.dress.imageUrl}
                    alt={item.dress.nameAr}
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <div className="text-sm text-white/60 mt-2 space-y-1">
                        <p>
                          المقاس: <span className="text-white">{item.size}</span>
                        </p>
                        <p>
                          اللون: <span className="text-white">{item.color}</span>
                        </p>
                      </div>
                    </div>
                    <button
                      onClick={() => removeFromCart(item.id)}
                      className="text-white/40 hover:text-red-500 p-2 transition-colors"
                    >
                      <Trash2 size={20} />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    <div className="flex items-center border border-[#d4af37]/30 rounded-lg">
                      <button
                        ...
                        className="w-10 h-10 hover:bg-[#d4af37]/20"
                      >
                        -
                      </button>
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#0b0b0b] rounded-2xl border border-[#d4af37]/30 p-8 sticky top-28 shadow-xl">
              <h2 className="text-2xl font-bold mb-6 border-b border-[#d4af37]/20 pb-4">
                ملخص الطلب
              </h2>
              <div className="space-y-4 mb-8">
                ...
                <div className="flex justify-between text-white/60">
                  <span>التوصيل</span>
                  <span className="text-[#d4af37]">مجاناً</span>
                </div>
              ...
              <Button
                size="lg"
                className="w-full h-14 rounded-xl font-bold bg-[#d4af37] text-black hover:bg-[#b8860b] transition-all"
